...
const PaymentInterface: React.FC = () => {
    ...
    const handleTotalPriceOrder = () => {
        ...
        const totalPrice = (1 - (vouchersList[indexVoucher].discount / 100)) * cartTotal;
        setTotal(totalPrice);
    }
    ...
    const onSubmit: SubmitHandler<form> = async (formData) => {
        if (user) {
            ...
            newOrder.order = {
                ...
                status: 0,
                total: 123,
                original_total: getTotalPriceCart(),
            };
            if (newOrder.order && vouchersList.length > 0) {
                newOrder.order.code = vouchersList[indexVoucher].code;
            }
            ...
            <div className="mx-auto">
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="flex gap-8 mt-5 justify-center">
                    ...
                    <div className="bg-white w-[40%] shadow-md rounded px-8 pt-6 pb-8 mb-4">
                        ...
                        {/*    <Select onValueChange={(value) => setIndexDelivery(parseInt(value))}>*/}
                        ...
                        {/*            <b>Giá vận*/}
                        {/*                chuyển</b>: {transformCurrency(deliveries[indexDelivery].price)} * {1}km*/}
                        {/*            = {transformCurrency(deliveries[indexDelivery].price * 1)}*/}
                        {/*        </div>*/}
                        {/*    )}*/}
                        ...
                        <div className="mt-10 mx-auto w-max">
                            {vouchersList && vouchersList.length > 0 ? (
                                    <div>
                                        <Select onValueChange={(value) => setIndexVoucher(parseInt(value))}>
                                            <SelectTrigger className="w-max mx-auto text-2xl outline-none">
                                                <SelectValue placeholder="Chọn mã giảm giá dành cho đơn hàng của bạn"/>
                                            </SelectTrigger>
                                            <SelectContent>
                                        ...
                                        {deliveries && (
                                            <div className="text-[1.6rem] mt-5">
                                                <b>Giảm
                                                    giá: </b>: {vouchersList[indexVoucher].discount} * {20000} = {transformCurrency(deliveries[indexDelivery].price)}
                                            </div>
                                        )}
                                    </div>
                                )
                            ...
                            <div className="mt-10 text-2xl text-center">
                                <b>Tổng giá trị đơn hàng:</b> {transformCurrency(122312)}
                            </div>
                            <Button type={'submit'} className="w-max mt-10 py-8 rounded-md mx-auto">Thanh toán</Button>
                        </div>
                    </div>
                </form>
